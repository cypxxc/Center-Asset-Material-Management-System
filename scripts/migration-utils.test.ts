...
test('migration numbers allow only the repository exact historical duplicate pairs', () => {
  ...
  assert.throws(
    () => validateAvailableMigrationNumbers([...historicalPairs[1], '00018_third.sql']),
  ...
  assert.throws(
    () =>
      validateAvailableMigrationNumbers(['00018_allow_staff_manage_metadata.sql', '00018_legacy_impostor.sql']),
...
test('migration filenames accept Supabase CLI timestamps after the legacy numbered chain', () => {
  assert.doesNotThrow(() =>
    validateAvailableMigrationNumbers([
      '00040_add_depreciation_fields.sql',
      '20260829160000_harden_profiles_bulk_import_audit.sql',
    ]),
  )
})
