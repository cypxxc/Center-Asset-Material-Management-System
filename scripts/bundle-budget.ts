...
function extractAssignedJson(source: string, assignmentTarget: string): string {
  const assignmentStart = source.indexOf(assignmentTarget)
  if (assignmentStart < 0) throw new Error('Dashboard manifest assignment is malformed')
  let jsonStart = assignmentStart + assignmentTarget.length
  while (/\s/.test(source[jsonStart] ?? '')) jsonStart += 1
  if (source[jsonStart] !== '=') throw new Error('Dashboard manifest assignment is malformed')
  jsonStart += 1
  while (/\s/.test(source[jsonStart] ?? '')) jsonStart += 1
...
function readDashboardEntryFiles(buildDir: string): string[] {
  ...
  const assignmentTarget = `globalThis.__RSC_MANIFEST[${JSON.stringify(dashboardManifestKey)}]`

  let manifest: unknown
  try {
    manifest = JSON.parse(extractAssignedJson(source, assignmentTarget))
  } catch (error) {
  ...
  let files: string[]
  if (entryJSFiles === undefined) {
    const clientModules = (manifest as { clientModules?: unknown }).clientModules
    if (!clientModules || typeof clientModules !== 'object' || Array.isArray(clientModules)) {
      throw new Error('Dashboard manifest entryJSFiles or clientModules is missing or malformed')
    }
    files = Object.values(clientModules).flatMap((clientModule) => {
      const chunks = (clientModule as { chunks?: unknown })?.chunks
      if (!Array.isArray(chunks) || chunks.length % 2 !== 0 || chunks.some((chunk) => typeof chunk !== 'string')) {
        throw new Error('Dashboard manifest entryJSFiles or clientModules is missing or malformed')
      }
      return chunks.filter((_, index) => index % 2 === 1) as string[]
    })
  } else {
    ...
    files = Object.values(entryJSFiles).flatMap((value) => {
  ...
  if (files.length === 0) throw new Error('Dashboard manifest entryJSFiles or clientModules is missing or malformed')
