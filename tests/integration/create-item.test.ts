...
test('createItem inserts attribution, delegates audit persistence to the database trigger, refreshes caches, and redirects', async () => {
  reset()
  staff()
...
test('each create action delegates item audit persistence to the database trigger', async () => {
  async function assertTriggerOwnedAuditFor(action: typeof createItem | typeof createItemInline) {
    reset()
    staff()
    mockSupabaseRegistry.setTableResponse('items', [{ id: 'new-item-uuid' }])
  ...
  await assertTriggerOwnedAuditFor(createItem)
  await assertTriggerOwnedAuditFor(createItemInline)
})
...
test('both actions reject cache failures after commit without removing the uploaded image', async () => {
  for (const action of [createItem, createItemInline]) {
    ...
    try {
      ...
      assert.equal(inserts.some(({ table }) => table === 'audit_logs'), false)
    } finally {
      cacheFailure = null
      restoreEnv('NEXT_PUBLIC_SUPABASE_URL', previousUrl)
      restoreEnv('SUPABASE_SERVICE_ROLE_KEY', previousKey)
